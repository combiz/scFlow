...
Differential gene expression was performed on a total of `r prettyNum(as.numeric(attr(metadata, "de_parameters")["n_genes"]),big.mark=",",scientific=FALSE)` genes which expressed a minimum of `r as.numeric(attr(metadata, "de_parameters")["min_counts"])` count in at least `r as.numeric(attr(metadata, "de_parameters")["min_cells_pc"])*100` percent of the cells. DGEs were detected at a fold-change threshold of `r as.numeric(attr(metadata, "de_parameters")["fc_threshold"])` and an adjusted p-value (padj) cut-off of `r as.numeric(attr(metadata, "de_parameters")["pval_cutoff"])` was applied. 
...
fc_threshold <- as.numeric(attr(metadata, "de_parameters")["fc_threshold"]) 

pval_cutoff <- as.numeric(attr(metadata, "de_parameters")["pval_cutoff"])
