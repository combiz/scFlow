...
sig_celltypes <- results$dirichlet_pvals %>% dplyr::filter(pval <= 0.05) %>% dplyr::select(!!results$fargs$celltype_var) %>% unique() %>% as.character()
```
...
`r if(nrow(results$dirichlet_pvals %>% dplyr::filter(pval <= 0.05)) > 0) {sprintf("Statistically significant differences in cell-type composition were observed in the following cell-types: %s", paste("<i>", sig_celltypes,"</i>", collapse = ", "))} else { print("No statistically significant differences in cell-type composition were observed.") }`
...
out <- NULL
celltypes <- unique(results$dirichlet_pvals[order(results$dirichlet_pvals$pval),][[results$fargs$celltype_var]]) #ordered by pvalue
...
DR_data$counts <- NULL
DT::datatable(DR_data, rownames = TRUE, options = list(pageLength = nrow(DR_data), scrollX=T, columnDefs = list(list(className = 'dt-left', targets = "_all"))), escape = FALSE) %>% DT::formatRound(columns = colnames(DR_data), digits = 3)
```
