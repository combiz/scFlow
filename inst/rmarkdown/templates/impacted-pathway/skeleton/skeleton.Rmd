...
`r if(length(setdiff(names(enrichment_result$WebGestaltR), "metadata")) == 0){"No significant impacted pathways found at FDR <= 0.05!"}`
...
`r if(length(setdiff(names(enrichment_result$ROntoTools), "metadata")) == 0){"No significant impacted pathways found at FDR <= 0.05!"}`

```{r conditional_rontotools_block, child='rontotools_template.Rmd', eval = !is.null(metadata$ROntoTools) && length(setdiff(names(metadata$ROntoTools), "metadata")) > 0}
```
...
`r if(length(setdiff(names(enrichment_result$enrichR), "metadata")) == 0){"No significant impacted pathways found at FDR <= 0.05!"}`
