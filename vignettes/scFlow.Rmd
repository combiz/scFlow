...
scFlowExamples::write_data(dataset="indvExp_ds_4K", output_dir=outputDir)
scFlowExamples::write_scflow_manifest(dataset="indvExp_ds_4K", output_dir=outputDir)
scFlowExamples::write_scflow_samplesheet(dataset="indvExp_ds_4K", 
...
ctd_fp <- outputDir
download.file(url = "https://raw.githubusercontent.com/combiz/scFlowData/master/inst/extdata/ensembl_mappings.tsv", destfile = ensembl_fp)
download.file(url = "https://github.com/neurogenomics/MAGMA_Celltyping/releases/download/latest/ctd_allAIBS.rds", destfile = file.path(ctd_fp, "ctd_allAIBS.rds"))
```
...
sce <- map_celltypes_sce(sce,
                         ctd_folder = ctd_fp, annotation_level = 2)
```
...
```{r eval=TRUE, message=TRUE}
plot_reduced_dim(sce, feature_dim = "cluster_celltype",
                 reduced_dim = "UMAP_Liger", alpha = 1, size = 1)
```
...
We have implemented two different functional enrichment tools enrichR and WebGestaltR. Users can call both tools using `find_impacted_pathway()` function. Here we will use WebGestaltR only.

```{r, message=FALSE, warning=FALSE}
# Get the significant genes for impacted pathway analysis

sig_de <- result_de$Controls_vs_diagnosisCases %>%
...
```{r eval=TRUE, message=FALSE, include=TRUE}
DT::datatable(enrichment_result$geneontology_biological_process_noredundant,
              rownames = FALSE, 
              escape = FALSE,
              options = list(pageLength = 5, 
                             scrollX=T, 
