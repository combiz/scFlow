...
#'
#' Make a Liger object:
#' @param raw.data List of expression matrices (gene by cell) each from a SingleCellExperiment object.
...
#' @param num.genes Number of genes to find for each dataset. Set to 3000 as default.
...
liger_reduce_dims <- function(sce_merged,
							take.gene.union = F, 
                            remove.missing = T,
							num.genes = 3000,
							combine = "union",
  ...
  # Split merged sce object into multiple sce objects and extract sparse matrices
  datasetList <- list()
  matList <- list()
  manifests <- unique(sce_merged@colData$manifest)
  for(mnft in manifests){
		datasetName <- paste0("dataset_", mnft)
		datasetList[[datasetName]] <- sce_merged[,sce_merged@colData$manifest == mnft]
		matList[[datasetName]] <- sce_merged@assays$data$counts[,sce_merged@colData$manifest == mnft]
  }
  
  # Make a Liger object. Pass in the sparse matrix.  
  ligerex <- createLiger(raw.data = matList, take.gene.union = take.gene.union, 
  ...
  ligerex <- liger::selectGenes(ligerex, num.genes = num.genes, combine = combine, keep.unique = keep.unique, 
                         capitalize = capitalize, do.plot = do.plot, cex.use = cex.use)
  
  sce_merged@metadata$liger_params$liger_selectedGenes <- 3000
